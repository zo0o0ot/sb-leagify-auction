  ...
  <AppShell>
    ...
    <template #sidebar-footer>
      <div class="p-4 space-y-2">
        <div class="text-[10px] font-label text-outline uppercase tracking-widest mb-3">All Team Budgets</div>
        <div
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="w-full h-1 bg-surface-container-highest mt-1">
              <div
                ...
                :style="{ width: `${Math.round((team.remaining_budget / (store.auction?.default_budget ?? 200)) * 100)}%` }"
              ></div>
            </div>
          </div>
          <span class="text-xs font-headline font-bold text-primary ml-3 flex-shrink-0">${{ team.remaining_budget }}</span>
        </div>
      </div>
    </template>

    <!-- Ticker -->
    <template #ticker-status>ROSTER VIEW: LIVE</template>
    ...
    <div v-else class="h-[calc(100vh-104px)] flex flex-col overflow-hidden">
      ...
      <!-- Team tabs -->
      <div class="flex items-end gap-0 border-b border-outline-variant/20 bg-surface-container-low px-6 overflow-x-auto flex-shrink-0">
        <button
          v-for="team in sortedTeams"
          :key="team.id"
      ...
      <div v-if="selectedTeam" class="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div class="grid grid-cols-3 gap-4">
          <div class="bg-surface-container border border-outline-variant/20 p-4 relative overflow-hidden">
            <div class="text-[10px] font-label text-outline uppercase tracking-widest mb-1">Budget Remaining</div>
            <div class="text-3xl font-headline font-black text-primary">${{ selectedTeam.remaining_budget }}</div>
            <div class="w-full h-0.5 bg-primary/20 absolute bottom-0 left-0">
              <div
                class="h-full bg-primary transition-all"
                :style="{ width: `${Math.round((selectedTeam.remaining_budget / (store.auction?.default_budget ?? 200)) * 100)}%` }"
              ></div>
            </div>
          </div>
          <div class="bg-surface-container border border-outline-variant/20 p-4 relative overflow-hidden">
            <div class="text-[10px] font-label text-outline uppercase tracking-widest mb-1">Budget Spent</div>
            <div class="text-3xl font-headline font-black text-tertiary">${{ teamSpent(selectedTeam.id) }}</div>
            <div class="w-full h-0.5 bg-tertiary/20 absolute bottom-0 left-0">
              <div
                class="h-full bg-tertiary transition-all"
                :style="{ width: `${Math.round((teamSpent(selectedTeam.id) / (store.auction?.default_budget ?? 200)) * 100)}%` }"
              ></div>
            </div>
          </div>
          <div class="bg-surface-container border border-outline-variant/20 p-4 relative overflow-hidden">
            <div class="text-[10px] font-label text-outline uppercase tracking-widest mb-1">Projected Points</div>
            <div class="text-3xl font-headline font-black text-on-surface">{{ teamProjectedPoints(selectedTeam.id) }}</div>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-2 gap-3">
          ...
          >
            ...
            <div class="w-14 h-14 flex-shrink-0 mr-4 flex items-center justify-center ml-2"
              :class="slot.pick ? 'bg-white p-1.5 shadow-lg' : 'bg-surface-container-highest border border-outline-variant/20'"
            >
              ...
              <span v-else-if="slot.pick" class="font-headline font-black text-surface-container-lowest text-lg">
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="flex items-center gap-2 mb-1">
                <span
                  ...
                  :style="{ backgroundColor: (slot.position.color_code ?? '#8d9199') + '33', color: slot.position.color_code ?? '#8d9199' }"
                >
              ...
              <template v-if="slot.pick">
                <div class="text-base font-headline font-black text-on-surface uppercase tracking-tight truncate">
              ...
              <div v-else class="text-sm font-label text-outline uppercase tracking-wider">Empty slot</div>
            </div>
            ...
            <div v-if="slot.pick" class="text-right flex-shrink-0 ml-3">
              ...
              <div class="text-lg font-headline font-bold text-tertiary">${{ slot.pick.winning_bid }}</div>
            </div>
          </div>
        </div>
      </div>

      <div v-else class="flex-1 flex items-center justify-center text-outline font-label uppercase text-sm">
        No teams found
      </div>
    </div>
  </AppShell>
